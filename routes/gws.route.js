...
router.get("/wines", async (req, res, next) => {
  ...
  if (previousSearchResultsFromDB) {
    ...
    console.log("No luck on finding the results in our DB. Making the request from the api now!")
    ...
    console.log("We got the results back from the api request! Storing them now for future reference!")
   ...
   console.log("Storing the search parameters from our request so we can keep track of our searches")
